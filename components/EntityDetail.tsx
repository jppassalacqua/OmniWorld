...
import { Map as MapIcon, Sparkles, Languages, X, Plus } from 'lucide-react';
import { Entity, Attribute, Relationship, SupportedLanguage } from '../types';
import { RichTextEditor, TagInput } from './Shared';
...
export const EntityDetail = ({ entity, allEntities, lang, onUpdate, onCreateEntity, onExport, setView, aiService }: EntityDetailProps) => {
    ...
    const handleGenerateImage = async () => { 
        if(confirm("Generate new image?")) { 
            ...
            try { 
                const desc = getLocalized(entity.description, lang) || getLocalized(entity.name, lang); 
                const newImg = await aiService.generateImage(desc, 'entity', entity.type); 
                if (newImg) onUpdate({ ...entity, imageUrl: newImg }); 
            } catch(e) { 
                alert("Failed"); 
            } finally { 
    ...
    return (
        <div className="h-full flex flex-col bg-slate-900 overflow-y-auto">
             <div className={`relative h-72 shrink-0 group ${isHovering ? 'brightness-110 ring-2 ring-indigo-500' : ''}`} onMouseEnter={() => setIsHovering(true)} onMouseLeave={() => setIsHovering(false)}>
                {entity.imageUrl ? <img src={entity.imageUrl} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center bg-slate-800 text-slate-600"><MapIcon size={64} opacity={0.5}/></div>}
                <div className="absolute inset-0 bg-gradient-to-t from-slate-900 via-transparent to-transparent" />
                <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={handleGenerateImage} disabled={isGeneratingImg} className="p-2 bg-slate-900/80 hover:bg-indigo-600 text-white rounded-lg backdrop-blur border border-slate-700 shadow-xl">{isGeneratingImg ? <div className="animate-spin w-4 h-4 border-2 border-white border-t-transparent rounded-full"/> : <Sparkles size={16}/>}</button>
                </div>
                <div className="absolute bottom-4 left-6 z-10 w-full pr-12">
                    <div className="flex items-end gap-3 mb-1">
                        ...
                        <button onClick={() => handleTranslate('name')} disabled={!!translatingField} className="mb-2 p-1.5 bg-indigo-600/80 hover:bg-indigo-500 text-white rounded shadow-lg backdrop-blur transition-all"><Languages size={14}/></button>
                        <span className="text-indigo-400 font-mono text-sm px-2 py-1 bg-indigo-900/30 border border-indigo-500/30 rounded mb-1.5">{entity.type}</span>
                    </div>
                </div>
             </div>
